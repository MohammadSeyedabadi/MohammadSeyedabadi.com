...
export function getPostData(postIdentifier, locale) {
  ...
  let filePath;
  if (locale === undefined) {
    filePath = path.join(postsDirectory, `${postSlug}.md`); // this construct a full absolute path to that specefic file
  } else {
    filePath = path.join(postsDirectory, `${locale + "-" + postSlug}.md`); // this construct a full absolute path to that specefic file
  }
